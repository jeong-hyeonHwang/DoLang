...
export const saveNote = async (note: NoteRequest) => {
  try {
    const response = await apiInstance.post('/api/note', note);
...
export const getNoteListPage = async (page?: number) => {
  try {
    const response = await apiInstance.get<{ result: NoteResponse[] }>(`/api/note?page=${page}`);
...
export const getNoteListWithKeyword = async (keyword: string) => {
  try {
    const response = await apiInstance.get<{ result: NoteResponse[] }>(`/api/note/search?keyword=${keyword}`);
    return response.data.result;
  } catch (error) {
